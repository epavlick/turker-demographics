...
def get_language_dicts_quals_assigns(path, qual_cutoff=None, strict=False, filter_list=None, match=True):
        all_dicts = dict()
        hitmap = hit_map()
        langids, langcodes = dat.lang_map()
        hitlangs = dat.hits_language()
...
def get_language_dicts_turkers(path, qual_cutoff=None, strict=False, filter_list=None, match=True):
	all_dicts = dict()
	tmap = turker_map()
	hitmap = hit_map()
	langids, langcodes = dat.lang_map()
	...
	for cnt, assign in enumerate(open(path).readlines()):
		comps = assign.strip().split('\t')
		aid = comps[0]
		if(aid == '' or aid not in tmap): continue
		tid = tmap[aid]
		if(tid not in tquals): continue
		#if match, filter_list is list of assignments to include, otherwise, filter_list is list of assignments to skip
		if(match): list_pass = not(filter_list == None) and (aid not in filter_list)
		else: list_pass = not(filter_list == None) and (aid in filter_list)
		if(list_pass): 
			print 'not in list', aid, tid
			continue
		#if strict, assignments must be greater than qual_cutoff to pass, otherwise, can be greater or equal to qual_cutoff	
		if(strict): qual_pass = not(qual_cutoff == None) and (tquals[tid] == 'N/A' or tquals[tid] <= qual_cutoff)
		else: qual_pass = not(qual_cutoff == None) and (tquals[tid] == 'N/A' or tquals[tid] < qual_cutoff)
		if(qual_pass):
			print 'below cutoff', aid, tquals[tid]
			continue
		alang =  langids[hitlangs[hitmap[aid]]]
		if not alang in all_dicts: all_dicts[alang] = dict()
		for pair in comps[1:]:
			try:
...
if __name__ == '__main__':
  	...
  	parser = argparse.ArgumentParser(description="Extract dictionaries from Mturk data")
        parser.add_argument('--quality', action='store_true', default=False, help='Dictionary processing based on turker quality')
        parser.add_argument('--cutoff', type=float, default=0.0, help='Turker quality cutoff')
        parser.add_argument('--strict', action='store_true', default=False, help='If quality equals cutoff, whether to accept or reject')
        parser.add_argument('--region', action='store_true', default=False, help='Dictionary processing based on turker region')
        parser.add_argument('--match', action='store_true', default=False, help='Use translations generated by in-region turkers')
        parser.add_argument('--nomatch', action='store_true', default=False, help='Use translations generated by out-of-region turkers')
        parser.add_argument('--all', action='store_true', default=False, help='Use translations generated by all turkers')
        parser.add_argument('--dictionaries', action='store_true', default=False, help='Compile word-to-word dictionaries')
        parser.add_argument('--turkerqual', action='store_true', default=False, help='Estimate quality of workers')
        parser.add_argument('--assignqual', action='store_true', default=False, help='Estimate quality of assignments')
        parser.add_argument('--turkercounts', action='store_true', default=False, help='Get counts of workers')
	...
	if args.quality:
		base = '%s/qual-cutoff/%.02f'%(DICT_DIR,args.cutoff,)
		if(args.dictionaries):
			prefix='%s/dictionary'%base
			print 'writing data to %s'%prefix
			if not(os.path.exists(base)): os.mkdir(base)
			dicts = get_language_dicts_turkers(DICT_PATH, qual_cutoff=args.cutoff, strict=args.strict)
	...
	if args.region:
		...
		if(args.dictionaries):
			...
			write_dicts(dicts, file_prefix=prefix)
		if(args.assignqual):
			print "Estimating assign quality by language"
			prefix='%s.assignqual'%base
			dicts = get_language_dicts_quals_assigns(DICT_PATH, qual_cutoff=0, strict=False,  filter_list=clpairs, match=mtch)
