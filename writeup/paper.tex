...
\caption{The number of workers per country.  This map was generated based on geolocating the IP address of the 5,284 workers in our study.  The size of the circles represents the number of worker from each country.  The two largest are India (2,022 workers) and the United States (904).  To calibrate the sizes: the Philippines has 147 workers, Egypt has 25, Russia has 10, and Sri Lanka has 4.}
\label{map}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Amazon's Mechanical Turk (MTurk) is an online marketplace for work which gives employers and researchers access to a large, low-cost, workforce. MTurk provides a simple interface for accessing human skills, allowing employers to provide micro-payments in return for workers completing micro-tasks.  The basic units of work on MTurk are called ``Human Intelligence Tasks'' (HITs).  MTurk was designed to accommodate tasks that are difficult for computers, but simple for people. This facilitates research into human computation, where people can be treated as a function call \cite{vonAhn2008,Little2009,quinn-bederson:2011}.  It has application to other fields like human-computer interaction \cite{bigham-et-al:2010,bernstein-et-al:2010}, computer vision  \cite{sorkin-forsyth:2008,deng-et-al:2010,rashtchian:10}, speech processing \cite{novotney-callisonburch:2010:NAACLHLT,Eskenazi:2013:crowdsourcing-speech-book},  and natural language processing \cite{Snow2008,callisonburch-dredze:2010:MTURK}. 
...
\newcite{irvine:10} Using Mechanical Turk to annotate lexicons for less commonly used languages

 MTurk to build parallel corpora for Machine Translation, a task which requires Turkers to speak two languages with a high level of proficiency.
\cite{ambati_act,ambati_naacl,bloodgood-callisonburch:2010:ACL,zaidan-callisonburch:2011:ACL-HLT2011a,Zbib-etal:2012:NAACL,post-callisonburch-osborne:2012:WMT}
