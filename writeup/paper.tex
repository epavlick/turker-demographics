...
\textit{In English, so people know what we did:}
\begin{align}	
	Quality = \frac{\# \text{translations in } Synonyms(\text{answer})}{\# \text{ controls}}
\end{align}	
where answer is the known translation and $Synonyms$(answer) is the set of words judged to be synonymous with the gold standard translation in the second pass HIT.
\textit{Mathified, to give the impression of rigor and intellect:}
\begin{align}	
	Quality = \frac{1}{k}\sum\limits_{i=1}^k\mathbf{1}(t_i \in Synonyms(a_i))
\end{align}	
where $k$ is the number of controls in the assignment, $t_i$ is the Turker's provided translation of control word $i$, $a_i$ is the gold standard translation of the control word $i$, and $Synonyms(a_i)$ is the set of words judged to be synonymous with the $a_i$ in the second pass HIT and $\mathbf{1}(x)$ is 1 when $x$ is true. 

Since most assignments had two known words embedded, most assignments had scores of either 0, 0.5, or 1. Since by-assignment quality scores are noisy and sensitive to small numbers of highly active Turkers, we report quality scores reported the average per-turker quality in tables and figures, where a Turker's quality is just the average quality of all the assignments that the Turker completed.
