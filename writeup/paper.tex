...
% Todo: sort by language code, rebalance the bottom set to have 19 languages in it.
\begin{table}[h]
\scriptsize 
\begin{center}
...
A set of gold standard translations were automatically harvested from Wikipedia  for every language to use as embedded controls. We used Wikipedia's inter-language links to pair titles of English articles with their corresponding foreign articles title.  To get a more translatable set of pairs, we excluded any pairs where: (1) the English word was not present in the WordNet ontology  \cite{miller1995wordnet}, (2) either article title was longer than a single word, (3) the English wikipedia page was a subcategory of person or place, or (4) the English and the foreign titles were identical or a substring of the other.
...
\begin{figure}[h]
\centering
%\includegraphics[height=\linewidth,angle=270]{figures/completiontime-series}
\includegraphics[height=\linewidth,angle=270]{figures/completiontime-series}
...
\paragraph{Performance within region}

We measured the average quality of workers who were in countries that plausibly speak a language, versus workers from countries that did not have large speaker populations of that language.  We used the Ethnologue \cite{ethnologue} to compile the list of countries for each language.  Table \ref{language-regions} reports the average translation quality of Turkers in countries with plausibly large speaker populations of each language, compared to the average quality of Turkers from outside the expected region for that language. 
