...
Each set of 10 words was independently translated by three separate workers.  5,281 workers completed 256,604 translations assignments, totaling more than 3 million words, over a period of three and a half months.
...
\begin{figure*}[t]
  ...
  \caption{An example of the variance in translation quality for the
    ...
    translations has been preserved exactly.}
  \label{figure:variance}
\end{figure*}

\paragraph{Full sentence translations}
In addition to the bilingual dictionaries, we are interested in Turkers' ability to translate complete sentences. Because the Indian subcontinent is so well represented among our workers, we chose six Indian languages for which to gather translations of full sentences. We selected Bengali, Malyalam, Hindi, Tamil, Telugu, and Urdu as good candidates for our study : these languages have large native speaker populations and are well represented on Wikipedia but they are currently not covered by any professionally translated corpora.
...
Figure~\ref{figure:variance} contains some hand-picked examples of the sorts of translations we obtained. While the lack of a gold-standard data set prevented us from evaluating the collected translations in terms of BLEU scores, as in \newcite{zaidan-callisonburch:2011:ACL-HLT2011a}, we evaluate the quality of the data by using it to train an SMT system. We present results in section \ref{sec:data-analysis}.


\section{Measuring Translation Quality}

We calculate the quality of translations on the level of individual assignments and aggregated over workers and languages.  We define an assignment's quality as the proportion of controls that are correct in a given assignment, where correct means exactly correct or judged to be synonymous.
\begin{align}	
...
\end{align}	
When a turker completed assignments in more than one language, their quality was computed separately for each language.


Figure \ref{hitlangqual} shows the translation quality for languages with contributions from at least 50 workers.  
...
\includegraphics[width=\linewidth]{../version1-figures/turker-googmatch-distribution.png}
...
\includegraphics[width=\linewidth]{../version1-figures/google-cdf-googlangs.png}
...
One obvious way for dishonest workers to shortcut is to use available online translation tools. Although we followed best practices to deter copying-and-pasting into online MT systems  by  rendering words and sentences as images \cite{zaidan-callisonburch:2011:ACL-HLT2011a}, this strategy does not prevent workers from typing the words into an MT system if they are able to type in the language's script. 
...
We found that while overall overlap is high (the average turker's overlap was 41\%), it is not consistently high across all turkers. Figure \ref{dist} shows that while a small number of turkers have a very high amount of overlap with Google (near 80\%) the majority overlap less than half of the time. It seems likely that those with very high overlap are cheating, but it is also reasonable to assume that honest turkers will overlap with Google with some regularity. We therefore divide the workers into three groups : those with very high overlap with Google (likely cheating by using Google to translate words), those with reasonable overlap, and those with no overlap (likely cheating by other means, e.g. submitting random text). 
...
\item Can Turkers' translations be used to train an MT system? 
\end{itemize}
...
\includegraphics[width=\linewidth]{../final-figures/completetime}
...
\hline\hline
Hindi & 0.635 (296) & \textbf{0.695} (7) & India (284) Uae (5) Uk (3)  & Saudiarabia (2) Russia (1) Oman (1) \\
Tamil & \textbf{0.653} (273) * & 0.250 (2) & India (266) Unitedstates (3) Canada (2)  & Tunisia (1) Egypt (1)  \\
Malayalam & 0.764 (234) & \textbf{0.827} (2) & India (223) Uae (6) Unitedstates (3)  & Saudiarabia (1) Maldives (1) \\
Spanish & 0.814 (191) & \textbf{0.837} (18) & Unitedstates (122) Mexico (16) Spain (14)  & India (15) Newzealand (1) Brazil (1) \\
French & 0.750 (170) & \textbf{0.816} (11) & India (62) Unitedstates (45) France (23)  & Greece (2) Netherlands (1) Japan (1) \\
Chinese & \textbf{0.601} (116) & 0.552 (21) & Unitedstates (75) Singapore (13) China (9)  & Hongkong (6) Australia (3) Germany (2)  \\
German & \textbf{0.825} (91) & 0.768 (41) & Germany (48) Unitedstates (25) Austria (7)  & India (34) Netherlands (1) Greece (1)  \\
Italian & \textbf{0.863} (90) & 0.796 (42) & Italy (42) Unitedstates (29) Romania (7)  & India (33) Ireland (2) Spain (2)  \\
...
Arabic & \textbf{0.741} (60) * & 0.604 (45) & Egypt (19) Jordan (16) Morocco (9)  & Unitedstates (19) India (11) Canada (3)  \\
Sindhi & \textbf{0.190} (96) & 0.062 (9) & India (58) Pakistan (37) Unitedstates (1)  & Macedonia (4) Georgia (2) Indonesia (2)  \\
Portuguese & 0.869 (101) & \textbf{0.963} (3) & Brazil (44) Portugal (31) Unitedstates (15)  & Romania (1) Japan (1) Israel (1) \\
Turkish & 0.759 (76) & \textbf{0.804} (27) & Turkey (38) Unitedstates (18) Macedonia (8)  & India (19) Pakistan (4) Taiwan (1) \\
Telugu & \textbf{0.799} (102) & 0.500 (1) & India (98) Unitedstates (3) Uae (1)  & Saudiarabia (1)  \\
Irish & \textbf{0.742} (54) & 0.709 (47) & Unitedstates (39) Ireland (13) Uk (2)  & India (36) Romania (5) Macedonia (2)  \\
Swedish & \textbf{0.734} (54) & 0.711 (45) & Unitedstates (25) Sweden (22) Finland (3)  & India (23) Macedonia (6) Croatia (2)  \\
Czech & \textbf{0.706} (45) & 0.612 (50) & Unitedstates (17) Czechrepublic (14) Serbia (5)  & Macedonia (22) India (10) Uk (5)  \\
Russian & \textbf{0.146} (67) & 0.119 (27) & Unitedstates (36) Moldova (7) Russia (6)  & India (14) Macedonia (4) Uk (3)  \\
...
Our workers reported speaking 95 languages natively. US workers alone reported 61 native languages. Overall, XXXX workers were located in a region likely to speak the language from which they were translating, and XXXX workers were located in countries considered out of region (meaning almost half of our 5,281 Turkers completed HITs in multiple languages). 
...
Higher overlap with Google Translate corresponds slightly to higher quality scores (Pearson $\rho$ = 0.XXX), suggesting that some control translations have already been indexed by Google. We therefore investigated the validity of our use of Wikipedia inter-language links as gold standard translations.
...
\paragraph{Quality improvements for SMT systems}
Using the full sentence translations we gathered for the six Indian languages, we were able to train a statistical machine translation system. Table \ref{table:data-size} gives the size of the data set for each of these languages. Table \ref{dictionary_bleu} shows that the sentences alone produce decent translation performance, with BLEU scores as high as 19.22 for Urdu. We also see that adding the single-word dictionaries to the training set produces consistent performance gains, ranging from 1 to 5 BLEU points. Unfortunately, we do not have professionally translated data against which to compare the resulting performances. We do, however, report the BLEU scores achieved by Google Translate, where available. Given that the sizes of the data sets used to train Google's system are likely much greater than ours, we find these results encouraging as an indication of the potential of Turkers to provide high quality translations.
...
\begin{table}[t]
  ...
  \begin{tabular}{l|rr}
    ...
    Urdu & 1,356k (35k) & 20k \\
  \end{tabular}
  \caption{Size of data set (number of words) collected for each language. For full sentences, number of parallel sentences is shown in parentheses.}
...
\section{Discussion}
Mechanical Turk gives researchers access to a diverse set of bilingual workers, making it a promising resource for researchers and developers of multilingual systems. 
